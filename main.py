...
class Particle(pygame.sprite.Sprite):
    def __init__(self, pos, dx, dy, var):
        ...
        if var == 'fire':
            super().__init__(fire)
            self.fire = [load_image("sprites/particles/fire_red.png"), load_image("sprites/particles/fire_orange.png"), load_image("sprites/particles/fire_yellow.png")] # \\
            for scale in (1, 2, 4, 8, 12, 16):
                self.fire.append(pygame.transform.scale(self.fire[random.choice([0, 1, 2])], (scale, scale)))
        if var == 'steel':
...
def create_particles(position, var):
    ...
    for _ in range(particle_count):
        Particle(position, random.choice(width_fire), random.choice(height_fire), var)
...
enemies = [load_image('sprites/playermodels/a.png'), load_image('sprites/playermodels/b.png'), load_image('sprites/playermodels/c.png')] # \\
